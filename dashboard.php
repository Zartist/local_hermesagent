...
// Rewrite responses: fix asset paths and set base path for the SPA
if (strpos($path, '/api/') !== 0 && $http_code === 200) {
    ...
    // For CSS responses, rewrite url() references (fonts, images)
    if (strpos($path, '/assets/') === 0 && substr($path, -4) === '.css') {
        $response_body = str_replace('url(/assets/', 'url(' . $proxy_base_path . '/assets/', $response_body);
        $response_body = str_replace('url(/fonts-terminal/', 'url(' . $proxy_base_path . '/fonts-terminal/', $response_body);
    }

    // For the HTML root page, rewrite tag attributes and set SPA base path
